...
public class addItems {
	...
	public void setSender(String mainCorpName,String subCorpName) 
	{
		...
		//dropdown.selectByVisibleText("æÒÇÑÉ ÇáÚãá");
		mainCrp.click();
		...
		//WebElement gehaMain = browserDriver.findElement(By.xpath("//li[contains(.,'ÇÇáßáíÉ ÇáÊÞäíå ÈÚäíÒÉ')]"));/*.findElement(By.id("select2-mainCorporateCustomers-container"));*/ 
		//gehaMain.click();
		/*for (WebElement option : options) {
			if (option.getText().equalsIgnoreCase("æÒÇÑÉ ÇáÚãá")) {
				option.click();
			}
		}*/		
	}
	...
	public void editItems(String[]itemsNos) throws InterruptedException  {
		...
		for (int i = 0; i < itemsNos.length; i++) {
			try {
				...
				genCls.bringTableRow("ItemsDataTable", currentExcelRow.get(0).toString()).findElement(By.cssSelector("button[data-original-title='ÊÚÏíá']")).click();
				Thread.sleep(1000);
