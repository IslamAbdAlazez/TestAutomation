...
public class deleteItems {
	...
	public void deleteItems(int itemsCount, boolean pageSource, boolean delPermOrFromStat) {
		...
		/*
		 * Select itemsLengthMenu = new
		 * Select(browserDriver.findElement(By.id("itemsLengthMenu")));
		 * itemsLengthMenu.selectByVisibleText(Integer.toString(65));
		 */

		for (int i = 0; i < itemsCount; i++) {
			WebElement currDelBtn = genCls.bringTableRow("ItemsDataTable", "")
		...
		for (int i = 0; i < itemsNos.length; i++) {
			...
			if (currentRow != null) {
				...
				if (pageSource) {
					...
					///// HERE I WILL BUT THE CODE THAT WILL CLICK ON DELETE PERMENTALLY OR DELETE
					///// FROM STATEMENT BUTTON
					if (delete[i]) {
						browserDriver.findElement(By.id("deleted")).click();
						currentRow = null;
					} else {
	...
	public static void main(String[] args) {
		...
		// String[] itemsNos = new String[2]; /*You can modify this number to be the
		// count of specific items You want to delete but make sure it is = the addition
		// in the next line*/
		// itemsNos[0] = "O1985010007SA"; itemsNos[1] = "O1985010003SA";
		// boolean [] delete = new boolean[3]; // This array is used to identify that
		// each of the items will be deleted perementaly or only deleted from the
		// statement
		// delete[0] = true; delete[1] = false; delete[2] = true;
		// delItems.deleteSomeItems(itemsNos,delete,true /*True mean this page is
		// invoked from register item page while false means this page is invoked from
		// the create statement page*/);
	}
}
